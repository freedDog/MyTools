package com.codemaker.resource;
...
import com.fd.utils.ObjectUtils;
...
public class Config{
  ...
  public boolean reload(String filePath)
  {
    try
    {
      int li = filePath.lastIndexOf(".properties");
      if (li > 0) {
    ...
    try
    {
      bundle = ResourceBundle.getBundle(this.filePath, locale);
    }
    catch (Exception e)
    {
      File file = new File(filePath + ".properties");
      System.err.println("nofind config:" + file.getAbsolutePath());
      return false;
    }
  ...
  public <T> T get(String key, Class<T> clazz)
  {
    ...
    if (value == null) {
      return (T)ObjectUtils.defualtValue(clazz);
    }
    return (T)ObjectUtils.baseValue(value, clazz);
  }
  
  protected <T> T get0(String key, Class<T> clazz)
  {
    String value = this.map != null ? (String)this.map.get(key) : null;
    if (value == null) {
      return null;
    }
    return (T)ObjectUtils.baseValue0(value, clazz);
  }
